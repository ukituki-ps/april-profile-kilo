...
- Статус: ✅ выполнено
- Задача: split `profiles-widget` — самостоятельный npm-виджет детальной карточки и создания + составной `ProfilesWidget`; доки без отдельного `profiles-widget-list.md`
- Ветка: `feature/065-profiles-widget-split-detail-composition` (локальный коммит; merge через PR в `develop`)
- Коммиты: один коммит в ветке `feature/065-profiles-widget-split-detail-composition` (сообщение начинается с `feat(profile-ui): виджет детальной карточки профиля…`)
- PR: не создавался

## 2) Что сделано

- **[frontend / `@april/profile-ui`]** Вынесен **`ProfilesWidgetProfileDetailCore`** (карточка + модалка создания, `forwardRef` → **`openCreate` / `closeCreate`**). Публичные фасады **`ProfilesWidgetProfileDetail`** и **`ProfilesApiWidgetProfileDetail`**. **`ProfilesWidgetCore`** остаётся сборкой: **`CardListColumn`** + детальная карточка; кнопка Add в списке вызывает **`detailRef.current.openCreate()`**. Удалён временный проп **`layout`** / тип **`ProfilesWidgetLayout`** (замена: **`ProfilesWidgetProfileDetail`**). Пропы **`documentEditingEnabled`** и **`allowProfileDelete`** гейтят редактирование/историческое сохранение/удаление в UI детальной карточки.
...
- **[телеметрия]** События виджета детальной карточки по-прежнему используют **`widget: "profiles_list"`** (тип `ProfileWidgetTelemetryKind` не расширялся; совместимость с существующими дашбордами). Описано в спеке `profiles-widget-profile-detail.md`.
...
- Телеметрия не разделяет сборку и детальную карточку в режиме standalone по полю **`widget`** — при необходимости follow-up: расширить `ProfileWidgetTelemetryKind` и договориться с потребителями событий.

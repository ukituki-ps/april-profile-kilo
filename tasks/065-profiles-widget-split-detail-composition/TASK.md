# `@april/profile-ui`: самостоятельный виджет детальной карточки профиля + сборка `profiles-widget` (список + детальная карточка + создание)
...
2. **`profiles-widget`** — **итоговый составной виджет**:
   ...
   - По **выбору строки** — в области детальной карточки монтируется **часть 1** нового виджета `profiles-widget-profile-detail` (или обёртка пакета с тем же контрактом данных).
...
4. **На будущее (в постановке зафиксировать, не реализовывать в объёме этой задачи):** другие виджеты (например вне `@april/profile-ui`) могут иметь **свой аналог** «детальной карточки профиля» с тем же UX-разбиением; в этой задаче — только **паттерн композиции** и публичный API для **профильного** контура.
...
- [ ] В `@april/profile-ui` экспортируется **самостоятельный** виджет детальной карточки и создания (имя в PR), с **частью 1** (опционально без редактирования) и **частью 2** (создание), открываемой **не только** из UI части 1.

...
- [x] Checkpoint сохраняется и используется для идемпотентных батчей (повторный прогон не дублирует обработанные записи в смысле домена).
- [x] Метрика лага по `source_system` обновляется и документирована (имя метки, единицы).
- [x] Автотесты покрывают ключевую логику; `go vet`, `go test` зелёные.
- [x] После появления `GET /metrics` в сервисе — метрики видны на scrape (follow-up или в рамках задачи, если endpoint уже есть).
...
- [x] Создана страница `docs-site/docs/task-story-016-<slug>.md`.
- [x] Обновлён [`docs-site/docs/task-stories-overview.md`](../../docs-site/docs/task-stories-overview.md).
- [x] Объяснено простым языком: зачем checkpoint и лаг по источникам для эксплуатации.
- [x] Границы: без полной фазы 5, без финальных алертов Hub.
- [x] Ссылки на `tasks/016-phase-3-source-sync-checkpoints-lag-metrics/TASK.md`, `PLAN.md`, `REPORT.md`.

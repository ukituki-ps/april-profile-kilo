...
- [ ] Branch protection для интеграционной ветки (`develop`) включена **или** зафиксирован явный блокер/исключение в `REPORT.md` с владельцем решения.
- [ ] `SUBMODULES_TOKEN` (и прочие необходимые секреты для текущих workflows) настроены в GitHub; CI на `develop` проходит после merge.
- [ ] `APRIL_DEPLOY_ROOT` в GitHub Variables согласован с документацией и реальным путём на сервере; деплой workflow завершается успешно.
- [ ] Smoke после деплоя **описан** в репо и **выполнен на dev**; результат отражён в [`REPORT.md`](./REPORT.md) (команды, вывод, ограничения «до появления backend»).
- [ ] Целевые команды проверки по репозиторию (см. ниже) зелёные на ветке после изменений в доках.

...
- [ ] Воркер Asynq запускается тем способом, который задокументирован для репозитория.
- [ ] Периодическая задача ping выполняется и наблюдаема (тест или интеграционная проверка).
- [ ] Реализована ровно одна прикладная фоновая задача с осмысленным телом и тестовым покрытием.
- [ ] Compose/deploy обновлены настолько, чтобы локально поднять API + Redis + воркер.
...
- [ ] Создана/обновлена страница `docs-site/docs/` в формате `task-story-015-<slug>.md`.
- [ ] В [`docs-site/docs/task-stories-overview.md`](../../docs-site/docs/task-stories-overview.md) добавлена ссылка и статус.
- [ ] Простым языком: зачем фоновые задачи и связь с событиями/outbox.
- [ ] Границы: без DLQ/ретраев как в прод-политике, без синков по внешним системам.
- [ ] Ссылки на `tasks/015-phase-3-asynq-infra-ping-domain-task/TASK.md`, `REPORT.md`.

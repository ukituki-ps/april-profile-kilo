...
- **Follow-up:** при необходимости для хоста — явный проп **`initialEntityId`** / синхронизация выбора только извне, отдельный экспорт «только детальная карточка» без загрузки полного списка (если появится отдельный API).

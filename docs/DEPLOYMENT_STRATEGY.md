...
| `APRIL_NPM_READ_TOKEN` *(опционально)* | **Settings → Secrets and variables → Actions → Secrets** | PAT с **`read:packages`** для `npm.pkg.github.com`, если для установки **`@april/*`** в job `frontend` недостаточно `GITHUB_TOKEN` (например PR из форка или политика org). В [`.github/workflows/ci.yml`](../.github/workflows/ci.yml), [`bootstrap-ci.yml`](../.github/workflows/bootstrap-ci.yml) и [`runner-cache-warmup.yml`](../.github/workflows/runner-cache-warmup.yml) передаётся как `NODE_AUTH_TOKEN: secrets.APRIL_NPM_READ_TOKEN \|\| secrets.GITHUB_TOKEN`. Значения не коммитить. |

**Локальная разработка (`frontend/`):** когда в [`frontend/.npmrc`](../frontend/.npmrc) раскомментированы строки для GitHub Packages, задайте в окружении **`NODE_AUTH_TOKEN`** (см. [`.env.example`](../.env.example)); иначе `npm ci` не сможет скачать приватные пакеты `@april/*`.

...
| `GHCR_PULL_TOKEN` *(опционально, рекомендуется)* | **Settings → Secrets and variables → Actions → Secrets** | PAT для pull приватных образов `ghcr.io` на self-hosted runner (минимум `read:packages`). В [`.github/workflows/dev-deploy.yml`](../.github/workflows/dev-deploy.yml) используется для `docker login ghcr.io` перед `deploy.sh`; если секрет пуст, workflow использует `github.token` при наличии достаточных прав. |

...
- `@april/ui` (≥ **0.1.5**): `CardListColumn` для списка; **`DensityProvider`** в корне виджета; для поля **`document`** — вкладки **`formData`** (данные профиля) и **`schema`** (read-only опубликованная JSON Schema типа сущности через **`getEntityTypePublishedSchema`** / `GET /v1/entity-types/{id}` при наличии метода у провайдера). Внутри **formData**: **`AprilJsonTreeEditor`** (просмотр и исторические версии — только read-only) или **`EntityTypesDraftJsonEditor`** (редактирование: **Tree / Source**, **`AprilJsonCollectionTextEditor`**, **`AprilJsonValidationSummary`**) — тот же стек, что в **057**, минимальная клиентская валидация корня `{ "type": "object" }`. Поле **`name`** профиля — отдельный **`TextInput`**, при create объединяется с объектом документа.
...
- RJSF-форма документа (`AprilJsonSchemaForm`): [`../../../tasks/059-phase-7-profiles-widget-rjsf-document-form/TASK.md`](../../../tasks/059-phase-7-profiles-widget-rjsf-document-form/TASK.md) (зависит от [`058`](../../../tasks/058-phase-7-profile-ui-ds-json-profiles-widget-integration/TASK.md)).

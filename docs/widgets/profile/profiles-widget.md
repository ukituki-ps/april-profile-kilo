...
- `@april/ui` (≥ **0.1.5**): `CardListColumn` для списка; **`DensityProvider`** в корне виджета; для поля **`document`** — **`AprilJsonTreeEditor`** (просмотр и исторические версии — только read-only), **`AprilJsonCollectionTextEditor`** + **`AprilJsonValidationSummary`** в связке **Tree / Source** через общий модуль **`EntityTypesDraftJsonEditor`** (тот же стек, что в задаче **057** для черновика схемы: минимальная клиентская схема валидации корня `{ "type": "object" }`, полная семантика — на сервере). Поле **`name`** профиля по-прежнему задаётся отдельным **`TextInput`** и при сохранении создаётся объединяется с объектом документа.

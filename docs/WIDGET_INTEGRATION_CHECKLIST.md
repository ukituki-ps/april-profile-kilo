...
- [ ] **Стек:** при открытии sheet/полноэкранного шага виджет **не** оставляет нижнюю панель родительского уровня активной параллельно дочерней (например `hideMobileShellBar` у `CardListColumn`, см. [`widgets/profile/profiles-widget.md`](./widgets/profile/profiles-widget.md)). Вложенный **`AprilVaulBottomSheet`** (например список версий в `profiles-widget-profile-detail`) — **вершина стека**: собственная **`AprilMobileShellBar`** родителя в этой ветке **не** показывается, пока лист открыт (закрытие — шапка/жесты листа).

...
Заполните таблицу для **вашего** репозитория после копирования `april_templay`. В примерах ниже — нейтральные плейсхолдеры; в репозитории **april_templay** значения по умолчанию заданы под этот шаблон.

| Имя | Пример для заполнения | Назначение |
| --- | --------------------- | ---------- |
| `DEV_HOST` | `dev.example.com` | Публичный хост dev-стенда |
| `DEPLOY_ROOT` | `/opt/<repo>` | Каталог git-клона на сервере (например `/opt/april_templay`) |
| `DEPLOY_USER` | `deploy` | Пользователь ОС для SSH и runner |
| `GITHUB_REPO_SLUG` | `<org>/<repo>` | Репозиторий в `git@github.com:` |
| `RUNNER_LABEL_EXTRA` | `templay` | Доп. label self-hosted runner (вместе с `dev`) |

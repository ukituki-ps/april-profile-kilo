...
- **Публикация наружу** в этой задаче не реализована: строка помечается как **`published`** с `published_at` в той же транзакции как **заглушка** «локально обработано»; реальная фоновая обработка и очередь **Asynq** — в задаче `tasks/015-phase-3-asynq-infra-ping-domain-task/TASK.md`.
